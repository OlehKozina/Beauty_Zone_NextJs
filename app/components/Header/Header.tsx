...
const Header = ({ header }: { header: NavigationType }) => {
  ...
  return (
    <>
      <header className="sm:max-md:mt-[4.5rem] mt-14 md:mt-14 flex top-0 left-0 absolute w-full z-10">
        <div className="w-full max-w-[80rem] mx-auto">
          <div className="flex px-4 py-2 sm:p-4 rounded-b-xl flex-grow items-center gap-[40px] z-2 justify-around bg-primary-light">
            <nav className="flex items-center justify-between flex-grow gap-10">
              ...
              <ul className="hidden sm:flex sm:flex-grow sm:justify-evenly sm:gap-[5px]">
                {!!navigation?.length &&
                  navigation.map((link) => (
                    <li
                      key={link.title}
            ...
            <button type="button" onClick={openMenu}>
              <FontAwesomeIcon
                icon={faBars}
                className="bg-transparent text-white w-6 h-6 sm:hidden hover:text-secondary-light"
              />
            </button>
          </div>
        </div>
